...
public class UNOWindow extends JFrame implements ActionListener, MouseListener {
	...
	private ControladorJugadores controladorJugadores = new ControladorJugadores(); 
	JPanel panelGame = new JPanel();
	JLabel lblBackground = new JLabel("");
	JPanel panelControls = new JPanel();
	JPanel panelAutoPlay = new JPanel();
	...
	private JToggleButton btnConfigure = new JToggleButton();
	private JToggleButton btnStep = new JToggleButton();
	private JToggleButton btnAuto = new JToggleButton();
	...
	public UNOWindow() 
	{
		...
		panelAutoPlay.setOpaque(false);
		panelAutoPlay.setBounds(440, 238, 80, 172);
		;
		contentPane.add(panelAutoPlay);
	...
	private void createAutoplayPanel()
	{
		
		panelAutoPlay.setLayout(new GridLayout(3, 1));
		
		btnConfigure.setOpaque(false);
		btnConfigure.setBackground(Color.WHITE);
		btnConfigure.setActionCommand("Configurar");
		btnConfigure.setIcon(new ImageIcon("./data/buttons/setup-button.png"));
		btnConfigure.addActionListener(this);
		panelAutoPlay.add(btnConfigure);
		
		btnStep.setOpaque(false);
		btnStep.setBackground(Color.WHITE);
		btnStep.setActionCommand("Jugada");
		btnStep.setIcon(new ImageIcon("./data/buttons/step-button.png"));
		btnStep.addActionListener(this);
		panelAutoPlay.add(btnStep);
		
		btnAuto.setOpaque(false);
		btnAuto.setBackground(Color.WHITE);
		btnAuto.setActionCommand("Automatico");
		btnAuto.setIcon(new ImageIcon("./data/buttons/auto-button.png"));
		btnAuto.addActionListener(this);
		panelAutoPlay.add(btnAuto);
	...
	public void actionPerformed(ActionEvent arg0) 
	{
		...
		else if(comm.equals("Automatico"))
		{
			//System.out.println("Automatico");
			
			//Validacion de todos los jugadores automaticos
			if ( !controladorJugadores.esPosibleJuegoAutonomo())
			{
				javax.swing.JOptionPane.showMessageDialog(this,"Para usar esta opcion es necesario que todos los jugadores esten configurados como automaticos");
			}
			else
			{
				//arrancar un thread que hace la jugada
				new Thread(){
					public void run() {
						while(!fachada.validarGanador())
						{
							controladorJugadores.jugar( fachada.darTurno(), fachada);
							jbPasar.setEnabled(false);
							jbRobar.setEnabled(true);
							String colorTexto = fachada.darTextoColorActual();
							setTitle("UNO - Juega "+colorTexto);
							lblBackground.setIcon(new ImageIcon("./data/bg/background-"+colorTexto+".jpg"));
		...
		else if(comm.equals("Configurar"))
		{
			//System.out.println("Configurar");
			PlayerSetupDialog configuracion = new PlayerSetupDialog(this.controladorJugadores);
	...
	private void delay(int milsecs)
	{
		try {
