...
public class UNOModelo 
{
	 ...
	 * conjuntoCartas
	 * int[][]
	 * Representacion del tablero de juego, matr�z entera de 4X27 donde cada valor representa el lugar donde est� la carta representada0
	 ...
	 *
	 *
	 * METODOS QUE DEBEN SER IMPLEMENTADOS POR EL ESTUDIANTE
	 *
	 *
	 */
	...
	/**
	 * reiniciarJuego
	 * El metodo tiene la responsabilidad de iniciar un juego nuevo limpiando el tablero (via llamado al correspondiente0
	 * metodo), repartir siete cartas a cada jugador y poner una carta en el mazo de descarte.
	 */
	...
	public void darCartasAlJugador(int jugador, int cuantas)
	{
		int i, j, k;
		for(k=0;k<cuantas;k++){
			i = darPosicionAleatoria(NUMERO_CARTAS);
			j = darPosicionAleatoria(NUMERO_COLORES);
			while(conjuntoCartas[i][j]!=CARTA_NO_USADA){
				i = darPosicionAleatoria(NUMERO_CARTAS);
				j = darPosicionAleatoria(NUMERO_COLORES);
			}
			conjuntoCartas[i][j] = jugador;
		}
	...
	{
		int ret = turnoActual + sentido;
		if(salto)
			ret = ret + sentido;
		if(ret==0)
			return NUMERO_JUGADORES;
		if(ret==-1)
			return NUMERO_JUGADORES-1;
		if(ret==NUMERO_JUGADORES+1)
			return 1;
		if(ret==NUMERO_JUGADORES+2)
			return 2;
		return ret;
	}
	/**
	 ...
	 * La matriz debe almacenar las posiciones i,j de la ubicacion de cada una de las cartas.
	 * 
	 */
	public int[][] darListadoCartasJugador(int jugador)
	{
		int cartas = contarCartasJugador(jugador);
		int mat[][] = new int[cartas][2];
		int conteo = 0;
		int i, j;
		for(i=0;i<NUMERO_CARTAS;i++)
			for(j=0;j<NUMERO_COLORES;j++)
				if(conjuntoCartas[i][j]==jugador){
					mat[conteo][0] = i;
					mat[conteo][1] = j;
					conteo++;
				}
	 ...
	 * 
	 * 
	 *FIN DE LOS METODOS QUE DEBEN SER IMPLEMENTADOS POR EL ESTUDIANTE
	 *
	 *
	 */
	 ...
	 */
	private int darPosicionAleatoria(int limite) 
	{
