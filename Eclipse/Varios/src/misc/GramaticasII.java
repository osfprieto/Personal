...
public class GramaticasII extends JFrame implements ActionListener{
	...
	private static int estado;//Nos dice en qu� parte del programa nos encontramos
	private static int cantidadSimbolos;//cantidad de s�mbolos que tiene el lenguaje
	private static int cantidadReglas;//cantidad de reglas que tiene la Gram�tica
	
	private static LinkedList<Character> simbolos;//Lista encadenada donde se almacenan los s�mbolos del lenguaje
	private static LinkedList<String> reglasImplicantes;//Lista encadenada donde se almacena la parte izquierda de cada una de las reglas de la gram�tica
	private static LinkedList<String> reglasImplicadas;//Lista encadenada donde se almacena la parte derecha de cada una de las reglas de la gram�tica
	
	private static LinkedList<JTextField> textSimbolos;//Lista encadenaeda que almacena los campos de texto mostrados en la interfaz gr�fica al momento de solicitar los s�mbolos del lenguaje
	private static LinkedList<JTextField> textReglasImplicante;//Lista encadenada que almacena los campos de texto mostrados en la intergaz gr�fica al momento de solicitar la parte izquierda de las reglase de la gram�tica
	private static LinkedList<JTextField> textReglasImplicadas;//Lista encadenada que almacena los campos de texto mostrados en la intergaz gr�fica al momento de solicitar la parte derecha de las reglase de la gram�tica
	
	private static Container cont;//Contenedor usado para darle formato a la ventana de la interfaz gr�fica
	
	private static JPanel panelButtons;//Panel en el que se ubicar�n los botones de comandos
	private static JPanel panelInputs;//Panel en el que se ubicar�n los campos de texto
	
	private static JButton aceptarButton;//Bot�n usado para dar la se�al de aceptar y continuar al siguiente estado del programa
	private static JButton cancelarButton;//Bot�n usado para terminar el programa en cualquier momento
	private static JButton agregarDatosButton;//Bot�n usado para agregar aumentar la cantidad de datos (s�mbolos o reglas) al momento del ingreso de estas seg�n sea el estado
	...
	public static void main(String[] args) {
		...
		/*inicializaci�n de las variables y muestra de la pimera ventana donde
		se solicitan la cantidad de s�mbolos y reglas iniciales*/
		...
		input.add(new JLabel("ingresar de s�mbolos y reglas:"));
		input.add(new JLabel("S�mbolos"));
	...
	/* Instancia todos los objetos referenciados en la clase como est�ticos
	 ...
	 * en los p�neles de la ventana y la actualiza para poder mostrar los cuadros
	 * de texto correspondientes al estado*/
	private void setUI(){
		if (estado == 0){//cuadro la interfaz del frame para mostrar los input de los s�mbolos
	...
	/* Dependiendo del estado agrega un objeto m�s a las listas de los s�mbolos o
	 ...
	 * sus correspondientes listas encadenadas de informaci�n y cambia el estado*/
	private boolean guardarDatos(){
		if(estado == 0){
			for(int i=0; i<cantidadSimbolos;i++){
				if(textSimbolos.get(i).getText().length()!=1){
					JOptionPane.showMessageDialog(null, "El s�molo "+(i+1)+" tiene m�s de un caracter", "Error", JOptionPane.ERROR_MESSAGE);
					return false;
				}
	...
	/* Revisa si un caracter es operador o n�mero para evitar que un estado
	 * se llame con estos s�mbolos reservados*/
	...
	@Override
	/*Revisa las acciones de los botones para el llamado a los m�todos*/
