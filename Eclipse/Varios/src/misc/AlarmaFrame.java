...
public class AlarmaFrame extends JFrame implements ActionListener{
	...
	public static void main(String[] args){
		isEstefany = false;
		String nombre = JOptionPane.showInputDialog("C�al es tu nombre?");
	...
	public void setup(){
		...
		while(horaNum<0 || horaNum>23 || minutoNum<0 || minutoNum>59 || utcNum<-12 || utcNum>12){
			...
			try{
				...
				if(isEstefany)
					JOptionPane.showMessageDialog(null, "Bb, algo pas� princesa :S", "Subs�", JOptionPane.INFORMATION_MESSAGE);
				else
	...
	public void alertar() throws Exception{
		//destruye el thread, modifica la GUI dando la opci�n de pausa adicionalmente
		//reproduce el sonido
		//muestra un mensaje en pantalla
		//sigue reproduciendo el sonido hasta que se cierre el mensaje y se
		//seleccione una opci�n de los botones
			...
			while(onWait){
				...
				if(horaActual==horaNum && minutoActual==minutoNum)
					try {
						...
						if(isEstefany)
							JOptionPane.showMessageDialog(null, "hmm algo pas� bb, toca reiniciar", "Ups", JOptionPane.INFORMATION_MESSAGE);
						else
							JOptionPane.showMessageDialog(null, "Una Excepci�n se ha presentado", "Error", JOptionPane.ERROR_MESSAGE);
						desactivar.doClick();
					}
			}
		}
