...
public class GramaticasI extends JFrame implements ActionListener{
	...
	public static void main(String[] args) {
		...
		while(!exito){
			try{
				cantidadSimbolos = Integer.parseInt(JOptionPane.showInputDialog("Ingresa la cantidad de símbolos a usar:"));
	...
	private void inicializar(){
		//inicio datos, variables, botones y páneles
		state = 0;
	...
	private void setUI(){
		if (state == 0){//cuadro la interfaz del frame para mostrar los input de los símbolos
		...
		else if (state == 1){//cuadro la intergaz del frame para mostrar los input de los símbolos
		...
		if(state == 0){
			for(int i=0; i<cantidadSimbolos;i++){
				if(textSimbolos[i].getText().length()!=1){
					JOptionPane.showMessageDialog(null, "El símolo "+(i+1)+" no tiene un dígito", "Error", JOptionPane.ERROR_MESSAGE);
					return false;
				}
				if(esOperador(textSimbolos[i].getText().charAt(0))){
					JOptionPane.showMessageDialog(null, "El símolo "+(i+1)+" es un operador o un número", "Error", JOptionPane.ERROR_MESSAGE);
					return false;
				}
